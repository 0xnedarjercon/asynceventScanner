...
from helpers.multiprocessingUtils import (
    Worker,
    SharedResult,
    JobManager,
    ContinuousWrapper,
...
class MultiWeb3(Logger):
    ...
    def mGet_logsCyclic(self, callback=None):
        ...
        for rpc in self.usedRpcs:
            ...
            while (
                len(rpc.jobs) <= self.numJobs and self.remaining[0] <= self.remaining[1]
            ):
    ...
    def mGet_logsLatest(self, *args, **kwargs):
        ...
        for rpc in self.usedRpcs:
            ...
            self.jobManager.addJob(
                "live", self.filter, 20, target=rpc.id, wait=False, *args, **kwargs
            )
