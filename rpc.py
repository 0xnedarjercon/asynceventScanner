...
class RPC(Logger):
        ...
        while self.live:
            try:
                ...
                if lastBlock < self.lastRxBlock:
                    lastRxBlock = await self.processNewEvents(
                        filter, remaining, jobLock, results, self.lastRxBlock
                    )
                    fails = 0
                    end = time.time()
        ...
        while self.live:
            try:
                ...
                if lastStoredBlock < self.lastRxBlock:
                    lastRxBlock = await self.processNewBlocks(
                        filter, remaining, jobLock, results, self.lastRxBlock
                    )
                    fails = 0
                    end = time.time()
    ...
    async def checkSocket(self, jobLock, remaining):
        payload = await asyncio.wait_for(
            self.w3.socket._manager._get_next_message(), 10
        )
        ...
        self.logInfo(
            f"message received {time.time()} blockTime{self.lastTimestamp} age: {self.lastTimestamp-time.time()}"
        )
        async with jobLock:
    ...
    async def doScan(self, remaining, filter, results, jobLock):
        ...
        try:
            ...
            self.logInfo(f'added results {filter["fromBlock"]}:{filter["toBlock"]}')
            self.failCount = 0
            self.throttle(result, filter["toBlock"] - filter["fromBlock"])
        except Exception as error:
            self.logInfo(f"error with job {type(error)} {error}")
            if self.failCount > 10 or (filter["toBlock"] - filter["fromBlock"]) <= 1:
                self.logWarn(f"too many failures, pausing rpc...")
                await self.returnJob(filter, remaining, jobLock)
                time.sleep(60)
            if self.failCount > 100:
    ...
    async def returnJob(self, filter, remaining, jobLock):
        self.currentJobs.append(filter)
    ...
    def handleInvalidParamsError(self, failingJob):
        if "Try with this block range" in failingJob[-1].args[0]["message"]:
            match = re.search(
                ...
                failingJob[-1].args[0]["data"],
            )
            if match:
                start_hex, end_hex = match.groups()
                suggestedLength = int(end_hex, 16) - int(start_hex, 16)
                self.logInfo(f"too many events, suggested range {suggestedLength}")
                self.splitJob(
    ...
    def handleError(self, failingJob):
        ...
        if type(e) == ValueError:
            if any(
                ...
                e.args[0]["message"] == "invalid params"
                or "response size should not greater than" in e.args[0]["message"]
            ):
            ...
            self.logWarn(f"timeout error, splitting jobs")
            self.splitJob(2, failingJob)
            self.failCount += 1
        elif type(e) == KeyboardInterrupt:
            pass
        # elif type(e) == Web3RPCError:
        #     if (
        #         "incorrect response body: wrong json-rpc response"
        #         in e.args[0]["message"]
        #     ):
        #         self.logInfo(f"inc resp error, trying again")
        #         self.splitJob(1, failingJob)
        #     else:
        #         self.logWarn(
        #             f"unhandled error {type(e), e},{traceback.format_exc()}  splitting jobs",
        #             True,
        ...
        #         self.failCount += 1
            ...
            self.failCount += 1
            self.logWarn(
                f"unhandled error {type(e), e},{traceback.format_exc()}  splitting jobs fails; {self.failCount}",
                True,
            )
            self.splitJob(2, failingJob)
    ...
    def splitJob(self, numJobs, failingJob, chunkSize=None, reduceChunkSize=True):
        ...
        if type(chunkSize) != (int):
            chunkSize = math.ceil(
                (failingJob[0]["toBlock"] - failingJob[0]["fromBlock"]) / numJobs
            )
        if reduceChunkSize:
